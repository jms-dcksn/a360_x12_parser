...
@BotCommand
@CommandPkg(label = "X12 to JSON",
		description = "Serialize X12 to JSON",
		icon = "x12_logo.svg",
		name = "getJSON",
		node_label = "Converts X12 format to JSON",
		group_label="Parse",
		comment = true,
		return_required = true,
		return_label = "Assign to list of strings",
		return_type = LIST,
		return_description = "Each entry in list contains a JSON string of the X12 file loop contents"
		)
	...
	public ListValue<StringValue> execute(
		...
		try {
			for (Iterator<Loop> nextLoop = loops.listIterator(); nextLoop.hasNext(); ) {
				intListOut.add(new StringValue(nextLoop.next().toJson()));
			}
